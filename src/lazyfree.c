...
/* Drop every key of a slot, in every database, by detaching the slot's hash
 * tables and releasing them in a background thread. This is O(1) in the number
 * of keys, unlike deleting them one by one.
 *
 * The caller is responsible for the notifications the keys would otherwise have
 * produced on their way out, see signalFlushedSlot(). */
